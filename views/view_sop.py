...
try:
    ...
    # Détermination des droits (RBAC)
    est_manager = user_info.get("role", "").upper() in ["MANAGER", "ADMIN", "CHEF_DE_SITE"]

    # Navigation Principale OPERA
    st.sidebar.title("Navigation OPERA")
    module_choisi = st.radio(
        "Sélectionner le domaine opérationnel :",
        ["🧭 Rondes & Patrouilles", "📜 Procédures & Consignes (SOP)"],
        horizontal=True
    )
    st.markdown("---")

    # Aiguillage selon le choix
