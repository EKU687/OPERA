...
class GenerateurProtocolePro(FPDF):
    ...
    def header(self):
        ...
        self.cell(0, 5, "GOUVERNEMENT DE LA NOUVELLE-CALÉDONIE", ln=True, align="R")
        self.set_font("helvetica", "", 9)
        self.set_text_color(100, 100, 100)
        self.cell(0, 5, "DIRECTION DES SÉCURITÉS — PROJET OPERA", ln=True, align="R")
        self.ln(6)
        ...
        self.cell(0, 8, f" PROTOCOLE : {self.site_nom.upper()} — {self.mission_titre.upper()} ({self.horaire})", fill=True, ln=True)
        self.ln(4)
    ...
    def footer(self):
        ...
        date_edition = datetime.datetime.now().strftime("%d/%m/%Y à %H:%M")
        self.cell(0, 10, f"Document Officiel OPERA — Généré le {date_edition} — Page {self.page_no()}/{{nb}}", align="C")
...
def creer_pdf(nom_site, mission, secteurs):
    ...
    for secteur in secteurs:
        ...
        pdf.cell(0, 7, f" SECTEUR : {secteur['nom_secteur'].upper()}", fill=True, ln=True)
        pdf.ln(2)
        ...
        for consigne in consignes:
            action = consigne['type_action'].upper()
...
# ==========================================
st.set_page_config(page_title="OPERA — Protocoles Sûreté", page_icon="🛡️", layout="wide")
...
try:
    ...
    # 🔒 SAS DE SÉCURITÉ PORTAIL GNC (Bloque l'exécution si hors portail)
    user_info = verifier_acces_depuis_portail(supabase)

    # En-tête personnalisé avec l'identité de l'agent
    ...
    # Filtrage des accès selon le rôle
    ...
    with tab_terrain:
        ...
        if sites:
            ...
            site_choisi = st.selectbox("📍 Sélectionner un site :", options_sites.keys(), key="select_terrain")
        ...
        with tab_creation:
            ...
            with st.expander("📍 1. Ajouter un nouveau Site", expanded=False):
                with st.form("form_site", clear_on_submit=True):
                    nom_nouveau_site = st.text_input("Nom du Site")
                    ...
                    if st.form_submit_button("Enregistrer le site"):
                        if nom_nouveau_site.strip():
                            ...
                            st.success(f"Site '{nom_nouveau_site}' ajouté.")
                            st.rerun()
            ...
            with st.expander("🕒 2. Ajouter une Mission", expanded=False):
                if sites:
                    ...
                    with st.form("form_mission", clear_on_submit=True):
                        titre_mission = st.text_input("Type de mission")
            ...
            with st.expander("🏢 3. Ajouter un Secteur (Zone)", expanded=False):
                if toutes_missions:
                    options_missions = {f"{next((s['nom_site'] for s in sites if s['id'] == m['site_id']), 'Site')} - {m['titre_mission']}": m['id'] for m in toutes_missions}
                    ...
                    with st.form("form_secteur", clear_on_submit=True):
                        nom_secteur = st.text_input("Nom du Secteur")
            ...
            with st.expander("📋 4. Ajouter une Consigne", expanded=False):
                if tous_secteurs and toutes_missions:
                    ...
                    for sec in tous_secteurs:
                        ...
                        if m_parent:
                            s_parent = next((s['nom_site'] for s in sites if s['id'] == m_parent['site_id']), "Site")
                    ...
                    with st.form("form_consigne", clear_on_submit=True):
                        ...
                        if st.form_submit_button("Enregistrer la consigne"):
                            if description.strip():
                                ...
                                st.success("Consigne enregistrée.")
                                st.rerun()

        # ONGLET 3 : MCO
        with tab_mco:
            st.write("**Maintien en Condition Opérationnelle**")
            ...
            if toutes_consignes and tous_secteurs and toutes_missions and sites:
                ...
                for con in toutes_consignes:
                    ...
                    if sec_p:
                        ...
                        if mis_p:
                            ...
                            nom_site = site_p['nom_site'] if site_p else "Site"
                ...
                if options_mco_consigne:
                    consigne_selectionnee = st.selectbox("Sélectionner l'action :", options_mco_consigne.keys())
                    ...
                    with col_m:
                        with st.form("form_edit_con"):
                            ...
                            nouvel_ordre = st.number_input("Ordre", value=obj_con['ordre_execution'], min_value=1)
